...
public class Biblioteca1 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            do {
                ...
                System.out.println("7. Cantidad de libros en prestamo.");
                System.out.println("8. Total personas que no han adquirido nunca"
                        + " un libro y empiezan por letra");
                ...
                switch (opcion) {
                    ...
                    case "6":
                        ...
                        break;
                    case "7":
                        mostrarLibrosEnPrestamo(con);
                        break;
                    case "8":
                        System.out.println("Introduce la inicial por la que buscar");
                        char inicial = entrada.next().charAt(0);
                        entrada.nextLine();
                        mostrarCantidadUsuariosNPInicial(con,inicial);
                        break;
                    case "S":
                    case "s":
    ...
    public static void mostrarUsuarios(Connection con) {
        try {
            Statement statement = con.createStatement();
            String sql = "SELECT cod, nombre FROM personas;";
    ...
    */
    ...
    public static void mostrarLibrosEnPrestamo(Connection con) {
        try {
            System.out.print("Cantidad de libros prestados: ");
            Statement statement = con.createStatement();
            String sentencia = "select ContarPrestamos();";
    ...
    public static void mostrarCantidadUsuariosNPInicial(Connection con, char letra) {
        
        try {
            
            CallableStatement cStmt = con.prepareCall("{call ContarUsuariosNPInicial(?)}");
            cStmt.registerOutParameter(1, Types.INTEGER);
            cStmt.setString(1, "" + letra );
            cStmt.execute();
            int resultado = cStmt.getInt(1);
            System.out.print("Total personas que no han adquirido nunca"
                    + " un libro y empiezan por "+letra + ": ");
            System.out.println(resultado);
            
            /*
            Statement statement = con.createStatement();
            String mostrarDatos = "SELECT ContarUsuariosNPInicial('"+letra+"');";
            ResultSet rs = statement.executeQuery(mostrarDatos);
            System.out.print("Total personas que no han adquirido nunca"
                    + " un libro y empiezan por "+letra + ": ");
            if( rs.next()){
